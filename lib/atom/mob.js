...
module.exports._symbols = {_observers, _client, _viewers, _viewing, _visible_tiles, _server_to_net, _add_viewing, _remove_viewing, _update_var, _common_tiles_count, _visgroups};
...
class Eye extends Component {
	...
	recalculate_visible_tiles() {
		...
		var new_visible = this.a.server[this.xray ? "compute_inrange_tiles" : "compute_visible_tiles"](this.atom, 8);
...
class Mob extends Component {
		...
		Object.defineProperty(this, 'eyes', {enumerable: true,configurable: false, writable: false, value: new Proxy({}, {
			set: (target, property, value) => {
				...
				if(value && this.client) {
					...
					this.client.next_message.eye[property] = {x:value.base_mover.x,y:value.base_mover.y,glide_size:value.base_mover.glide_size};
				}
				return true;
	...
	can_read_panel(atom, panel, key = "") {
		if(!this.basic_panel_read_checks(atom, panel, key))
			return false;
	...
	bind_panel(atom, panel, key = "") {
		if(!this.can_read_panel(atom, panel, key))
		...
		var check = () => {
			if(panel.is_open && this.can_read_panel(atom, panel, key)) {
