	...
	message_handler(data) {
		try {
			...
			if(obj.click_on) {
				...
				if(this.mob) {
					this.mob.c.Mob.emit("click_on", obj.click_on);
				}
	...
	send_network_updates() {
		...
		for(let netid in this[_atom_net_queue]) {
			...
			if(entry.create) {
				...
				for(var key of ['icon', 'icon_state', 'dir', 'layer', 'name', 'glide_size', 'screen_loc_x', 'screen_loc_y', 'overlays', 'x', 'y', 'opacity', 'flick'])
