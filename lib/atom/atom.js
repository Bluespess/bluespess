...
class Atom extends EventEmitter {
	...
	[_changeloc](newX, newY, newZ = this[_z], newLoc, newBounds_x, newBounds_y, newBounds_width, newBounds_height) {
		...
		var new_fine_loc = {x:newX, y:newY, z:newZ, dim:newLoc && newLoc.dim, loc:newLoc, is_fine_loc: true};
	...
	test_move(newX, newY) {
		...
		var gained_crossers = [];
		var common_crossers = [];
		var partial_uncross_x = [];
		var partial_uncross_y = [];
		...
		return {gained_crossers, lost_crossers, common_crossers, partial_uncross_x, partial_uncross_y};
	}
	...
	set loc(newLoc){
		...
		if(newLoc !== null && newLoc.is_fine_loc) {
			...
			if((newLoc.hasOwnProperty('z') && newLoc.z !== +newLoc.z) || newLoc.x !== +newLoc.x || newLoc.y !== +newLoc.y || !newLoc.dim)
	 ...
	 */
	get fine_loc() {
		return {x: this.x, y: this.y, z: this.z, loc: this.loc, dim: this.dim, is_fine_loc: true};
	}
	set fine_loc(val) {
		this.loc = val;
	}
	...
	move(offsetx, offsety, reason) {
		...
		for(let i = 0; i < move_splits; i++) {
			...
			if(clang)
				break;
			for(let common of result.lost_crossers) {
		...
		for(let i = 1; (i*Math.max(Math.abs(step_x),Math.abs(step_y))) >= (1/this.movement_granularity/2); i /= 2) {
			...
			if(!clang) for (let common of result.common_crossers) {
				if(!this.can_move_within(common, remaining_x, remaining_y, reason)) {
					clang = true;
					first_bump = null;
					first_bump_layer = -Infinity;
	...
	/**
	 * Whether this atom can move while crossing the given atom. Calls {@link Bluespess.Atom.can_crosser_move_within} on the atom moving within by default
	...
	* partial_locs() {
		...
		for(let x = Math.floor(this[_x]+this[_bounds_x]+0.00001); x < Math.ceil(this[_x]+this[_bounds_x]+this[_bounds_width]-0.00001); x++) {
			for(let y = Math.floor(this[_y]+this[_bounds_y]+0.00001); y < Math.ceil(this[_y]+this[_bounds_y]+this[_bounds_height]-0.00001); y++) {
				yield this.dim.location(x,y,this[_z]);
			}
		}
	}
