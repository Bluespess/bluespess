...
module.exports = class ChatMessage {
	...
	self(a, ...b) {
		...
		this.self(this.server.format_html(a, ...b));
	}
	...
	deaf(a, ...b) {
		...
		this.deaf(this.server.format_html(a, ...b));
	}
	...
	blind(a, ...b) {
		...
		this.blind(this.server.format_html(a, ...b));
	}
	...
	show_directly_to(target, source) {
		if(!target || !source || !this.server.has_component(target, "Hearer"))
			return;
		this.emitter = source;
