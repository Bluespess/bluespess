...
const FunctionChain = require('../functionchain.js');
...
class Atom extends EventEmitter {
	constructor(server, template, x, y, z) {
		...
		if(template.components) {
			for(let i = 0; i < template.components.length; i++) {
				...
				if(componentConstructor.depends)
					componentConstructor.depends.forEach((item) => {
						if(template.components.indexOf(item) == -1) {
							throw new Error(`Template '${template.id}' is missing dependency component '${item}' for component '${componentName}'`);
						}
					});
	...
	[_changeloc](newX, newY, newZ, newLoc, newBounds_x, newBounds_y, newBounds_width, newBounds_height) {
			...
			if(this[_loc].isBaseLoc) {
				for(let x = Math.floor(this[_x]+this[_bounds_x]+0.00001); x <= Math.ceil(this[_x]+this[_bounds_x]+this[_bounds_width]-0.00001); x++) {
					for(let y = Math.floor(this[_y]+this[_bounds_y]+0.00001); y <= Math.ceil(this[_y]+this[_bounds_y]+this[_bounds_height]-0.00001); y++) {
						...
						for(var atom of thisloc.partial_contents) {
		...
		this.emit("moved");
		this[_update_var]('x', 0); // Send the changes to the network.
		this[_update_var]('y', 0);
	}
	...
	[_update_var](varname, is_appearance) {
		for(let viewer of this[mob_symbols._viewers]) {
			viewer.components.Eye.enqueue_update_atom_var(viewer.components.Eye[mob_symbols._server_to_net][this.object_id], this, varname, is_appearance);
		}
	}
}
