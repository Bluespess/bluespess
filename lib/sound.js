...
class Sound {
	...
	play_to(mobs) {
		...
		var clients = new Set();
		for(let mob of mobs) {
			if(!this.server.is_atom(mob) && mob && mob.mob)
				clients.add(mob);
			if(!this.server.has_component(mob, "Eye"))
				continue;
			for(let observer of mob.c.Eye.observers()) {
				if(observer.c.Hearer.can_hear_sound(this))
					clients.add(observer.c.Mob.client);
			}
			if(this.server.has_component(mob, "Mob"))
				if(mob.c.Hearer.can_hear_sound(this))
	...
	emit_from(atom) {
		...
		for(let hearer of hearers) {
			if(this.server.has_component(hearer, "Mob") && hearer.c.Hearer.can_hear_sound(this)) {
