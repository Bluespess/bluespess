...
class Atom extends EventEmitter {
	...
	[_changeloc](newX, newY, newZ = this[_z], newLoc, newBounds_x, newBounds_y, newBounds_width, newBounds_height) {
		...
		for(let lost of lost_viewers) {
			lost.components.Eye[mob_symbols._common_tiles_count].set(this, lost.components.Eye[mob_symbols._common_tiles_count].get(this) - 1);
		}
		for(let gained of gained_viewers) {
			gained.components.Eye[mob_symbols._common_tiles_count].set(this, gained.components.Eye[mob_symbols._common_tiles_count].get(this) + 1 || 1);
		}
		for(let lost of lost_viewers) {
	...
	get x() {
		if(this[_loc] && this[_loc] && !this[_loc].isBaseLoc)
			return this[_loc].x;
		return this[_x];
	}
	set x(newX) {
		newX = +newX; // cast to number
		if(newX === this[_x] && this[_loc] && this[_loc].isBaseLoc)
			return;
	...
	set y (newY) {
		...
		if(newY === this[_y] && this[_loc] && this[_loc].isBaseLoc)
			return;
	...
	set z(newZ) {
		...
		if(newZ === this[_z] && this[_loc] && this[_loc].isBaseLoc)
			return;
	...
	set opacity(val) {
		...
		for(let crosser of this[_crosses]) {
			if(this.server.has_component(crosser, "LightingObject")) {
				crosser.components.LightingObject.update_shadow(this);
			}
		}
	}
