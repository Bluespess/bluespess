...
class Atom extends EventEmitter {
	...
	[_changeloc](newX, newY, newZ = this[_z], newLoc, newBounds_x, newBounds_y, newBounds_width, newBounds_height) {
		...
		if(old_fine_loc.loc && old_fine_loc.emit) {
			old_fine_loc.emit('before_exit', movement);
		}
		if(new_fine_loc.loc && new_fine_loc.emit) {
			new_fine_loc.emit('before_enter', movement);
		}
		if(newLoc && !newLoc.is_base_loc && this.loc && !this.loc.is_base_loc && newLoc == this.loc)
			return;
	...
	get x() {
		if(this[_loc] && this[_loc] && !this[_loc].is_base_loc)
			return this[_loc].x;
		return this[_x];
	}
	set x(newX) {
	...
	does_enclose_tile(tile) {
		if(!tile.is_base_loc || !this.loc || !this.loc.is_base_loc || this.z != tile.z)
			return false;
	...
	does_cross(atom, {x = this.x, y = this.y, z = this.z, bounds_x = this.bounds_x, bounds_y = this.bounds_y, bounds_width = this.bounds_width, bounds_height = this.bounds_height} = {}) {
		if(atom instanceof Atom && atom.z == z && this.loc && atom.loc && this.loc.is_base_loc && atom.loc.is_base_loc) {
	...
	get crosses() {
		return this[_crosses][Symbol.iterator].bind(this);
	}

	* partial_locs() {
		if(!this.loc.is_base_loc)
			return this.loc;
		for(let x = Math.floor(this[_x]+this[_bounds_x]+0.00001); x < Math.ceil(this[_x]+this[_bounds_x]+this[_bounds_width]-0.00001); x++) {
			for(let y = Math.floor(this[_y]+this[_bounds_y]+0.00001); y < Math.ceil(this[_y]+this[_bounds_y]+this[_bounds_height]-0.00001); y++) {
