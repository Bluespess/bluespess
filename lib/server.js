...
class Bluespess extends EventEmitter {
	...
	location(x,y,z) {
		...
		var id = `${x},${y},${z}`;
		if(this[_locations][id]) {
			return this[_locations][id];
		}
		var loc = new Location(x,y,z, this);
	...
	login(socket, username) {
		...
		if(this.dc_mobs[username]) {
			client.mob = this.dc_mobs[username];
			this.dc_mobs[username] = null;
		}
	}
	...
	compute_visible_tiles(atom, dist) {
		var visible_tiles = new Set();
		if(atom.loc == null) return visible_tiles;
		for(var x = Math.floor(atom.x + .00001 - dist); x <= Math.ceil(atom.x - .00001 + dist); x++) {
			for(var y = Math.floor(atom.y + .00001 - dist); y <= Math.ceil(atom.y - .00001 + dist); y++) {
				visible_tiles.add(this.location(x, y, atom.z));
			}
		}
		/*var rays = [];
		for(var i of [1,2,4,5,6,8,9,10]) {
			var ray = {dx: i & 4 ? 1 : i & 8 ? -1 : 0, dy: i & 1 ? 1 : i & 2 ? -1 : 0, atom, subrays:[]}
			rays.push(ray);
	...
	process_template(template) {
		...
		if(template.components) {
			...
			while(hasAddedDependencies) {
				...
				for(var componentName of template.components) {
...
class Location {
	...
	get_step(dir) {
		...
		if(dir & 8)
			newx--;
		return this.server.location(newx,newy,this.z);
	}
}
